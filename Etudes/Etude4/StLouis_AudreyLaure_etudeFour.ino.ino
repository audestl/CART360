...
void live()
{
  uint8_t val = analogRead(NOTE_IN_PIN);
  offsetFrequency = getPhotoFrequency();
  if (analogRead(NOTE_IN_PIN) < 10) // to get rid of noise
    noTone(BUZZER_PIN);
  else
    tone(BUZZER_PIN, val + offsetFrequency, 100);
  // output sound
...
void playCurrentNote()
{
  ...
  if(testNote>peak)
  peak=testNote;
  tone(BUZZER_PIN, peak + offsetFrequency, 100);
...
testNote = analogRead(NOTE_IN_PIN);
  ...
  notes[countNotes] = peak + averageOffsetFreq ;
...
int getPhotoFrequency()
{
  ...
  offsetFrequency = map(lightLevel, 250, 1000, 0, 100);
...
int getRunningAverage()
{
  ...
  runningAverageBuffer[nextCount] = getPhotoFrequency(); // store the photo frenquency in array
  nextCount++; // increase count of nextCount
...
void colorLED(int col)
{
  ...
  frq = map(col,0,1024,0,255);
  analogWrite(LED_PIN_R, 0);   
  analogWrite(LED_PIN_G, frq);   // Turn on the green LED based on the frequency of the note
...
for (int i=0; i < countNotes; i++)
{
  ...
  tone(BUZZER_PIN,notes[i],durations[i]); // output the notes
  colorLED(notes[i]);  // change the intensity of the LED
